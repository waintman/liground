...
아래의 이전 인계 기록을 바탕으로 `npm run dev`에서 배경 이미지 정렬 오류를 재현하고 수정했다.

- **원인:** `janggi`의 기본 이미지는 900×1000이지만 `janggimodern`/`janggicasual`의 기본 5개 스타일은 600×600 `*Coords.svg`를 쓴다. 이 이미지에는 오른쪽 여백이 포함되어 있다. 미리보기의 `background-size: 100% 100%`는 이 여백까지 180×200에 맞추므로 실제 판 교차점이 기물 격자와 달라진다. DOM의 판/기물 영역 크기만 검사하면 발견할 수 없다.
- **수정:** `PVLines.vue`의 배경 크기를 메인 보드와 같은 `cover`로 변경했다. `ChessGround.vue`에서는 장기 종류 변경 시 같은 스타일 이름을 유지하더라도 해당 종류의 스타일시트를 다시 불러온다. 기존에는 종류 전환 직후 이전 이미지가 남아 재현 여부가 달라졌다.
- **개발 화면 확인:** macOS, DPR 2, 렌더러 1728×475, `Ka_kakao` 기물. 현대 장기 darkwood에서 수정 전 오른쪽 흰 여백과 판 밖 기물을 확인했고, 수정 후 배경 교차점에 맞는 화면을 확인했다. 장기 3종 × darkwood/brown/dark/lightbrown/stone × 초/한 시점의 30개 조합에서 180×200 크기, 기물 좌표, FEN/기보 보존을 검사했다. 계산 격자와 기물 중심의 최대 오차는 약 0.071 CSS px였다.
- **검증 한계:** 실제 Fairy-Stockfish 분석에서 호버 중 미리보기 갱신과 FEN/기보 보존 검사까지 진행했으나, 후속 엔진 재시작 시 개발 렌더러가 새로고침되어 전체 엔진 검증을 완료하지 못했다. 사용자 지정 판 이미지, 다른 화면 배율, 메인 보드 리사이즈 후 검증은 별도로 필요하다.
- **완료한 검사:** `npm run lint`, `node tests/live-pv.test.cjs`, `npm run pack`, Fairy-Stockfish를 지정한 `npm run test:smoke` 모두 통과. 스모크의 renderer 오류는 0건이었다.
- 빌드·설정·의존성 파일과 기존 `.vscode/`는 변경하지 않았다. 아래 내용은 수정 전 인계 기록이다.
