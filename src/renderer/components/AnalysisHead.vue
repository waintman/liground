...
export default {
  ...
  emits: ['updateVariant', 'resetMultiEngine', 'startNewGame'],
  data () {
    return {
      selected: '♟️ Standard',
  ...
  methods: {
    ...
    async handleStart (payload) {
      ...
      await this.$store.dispatch('PvEfalse')
      await this.$store.dispatch('EvEfalse')
      await this.$store.dispatch('resetBoard', { is960: false })
      await this.$store.dispatch('setGameConfig', payload)
      ...
      if (isPvE) {
        ...
        await this.$store.dispatch('PvEtrue', {
          playerIsWhite,
        ...
        await this.$store.dispatch('EvEtrue', {
          gameMode: payload.gameMode,
